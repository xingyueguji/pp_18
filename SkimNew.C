...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (jentry %100000 == 0) cout << "We are at event " << jentry << " Percentage done: " << progress << "%" << endl;


      //Event selection:

      if (abs(zVtx) > 15) continue;
      ...
      for (int znum = 0; znum < Reco_QQ_size; znum++){
         ...
         if (nZs != Reco_QQ_size) cout << "Error: " << " Not Sure why Reco_QQ_Size != Size of TCloneArray" << endl;
         ...
         //Apply cuts
            //Cut on Z
         if (Z_momentum->M() < 60 || Z_momentum->M() > 120) continue;
         if (Reco_QQ_VtxProb[znum] < 0.001) continue;
         if (Z_momentum->Rapidity() > 2.4) continue;

            //Cut on Muons
         if (muonplus_momentum->Pt() < 20 || muonminus_momentum->Pt() < 20) continue;
         if (muonplus_momentum->Eta() > 2.4 || muonminus_momentum->Eta() > 2.4) continue;
         ...
         if (!isOppositeSign){
            if (Reco_QQ_sign[znum] == 0) cout << "You should never see this, charge matching failed!!!!" << endl;
            continue;
         }

            //Aco cut
         Float_t acoplanarity = 1 - TMath::Abs(TMath::ACos(TMath::Cos( muonplus_momentum->Phi() - muonminus_momentum->Phi() )))/TMath::Pi();
         if (acoplanarity < 0.001) continue;

            //Eta < 1 cut I will apply this later when filling
         Bool_t isEtacutPassed = muonplus_momentum->Eta() < 1 && muonminus_momentum->Eta() < 1;
         ...
         if (isEtacutPassed) h_mass_array_eta_inclusive->Fill(ZMass);
         if (isEtacutPassed) mass_array_eta_inclusive->add(RooArgSet(*x));
