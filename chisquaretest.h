...
chisquaretest::chisquaretest(TString s1, TString s2, TString s3){
	mcfilepath = s1;
	datafilepath = s2;
	bkfilepath = s3;
	...
	for (int runperiod = 0; runperiod < 22; runperiod++){
		...
		h_data_pp[runperiod] = (TH1D*) datafile ->Get(Form("mass_array_data_%i",runperiod));
	...
	for (int runperiod = 0; runperiod < 22; runperiod++){
		for (int shift = 0; shift < nbins_mass_shift; shift++){
			for (int smear = 0; smear < nbins_smear; smear ++){
				...
				if (isbk) chisquarevalue = h_data_bksub_pp[runperiod] -> Chi2Test(h_mc_signal_pp[shift][smear][runperiod],"WW CHI2/NDF");
				if (!isbk) chisquarevalue = h_data_pp[runperiod] -> Chi2Test(h_mc_signal_pp[shift][smear][runperiod],"WW CHI2/NDF");
				//cout << chisquarevalue << endl;
				//chisquarearray[shift][smear][cent] = chisquarevalue;
				std::ostringstream stream;
				stream << std::fixed << std::setprecision(2) << chisquarevalue;
...
void chisquaretest::plottingandformatting(int type, bool isbk){
	...
	for (int cent = 0; cent < nbins_cent; cent++){
		...
		if (cent == 0 || cent == 1 || cent == 2 || cent == 3 || cent == 4){
			...
			dMass_Err_HI[cent+1] = 0;
			dWidth_Err_HI[cent+1] = 0;
		}
	...
	TFile *temp = new TFile("pp_data_file_stability.root","UPDATE");
	temp->cd();
	if (isbk && type == 1){
		g_HI_dmass->Write("HI_dM_chi2_bksub",2);
		g_HI_dwidth->Write("HI_dWidth_chi2_bksub",2);
	}
	if (!isbk && type == 1){
		g_HI_dmass->Write("HI_dM_chi2",2);
		g_HI_dwidth->Write("HI_dWidth_chi2",2);
	}
	temp->Close();
...
void chisquaretest::plottingandformattingpp(bool isbk){
	...
	TString data_data_saving_path;
		...
		if (isbk){
			...
			chi2_saving_path = "./chi2pp/chi2plots/bksub/eta_with_bksub_%i.png";
			data_mc_saving_path = "./chi2pp/datamc/bksub/eta_with_bksub_%i.png";
			data_data_saving_path = "./chi2pp/datadata/eta_%i.png";
		} 
		if (!isbk){
			chi2_title = "Raw_without_bksub_%i";
			data_mc_title = "Raw_without_bksub_%i";
			data_data_title = "Raw_%i";
			chi2_saving_path = "./chi2pp/chi2plots/nobksub/eta_without_bksub_%i.png";
			data_mc_saving_path = "./chi2pp/datamc/nobksub/eta_without_bksub_%i.png";
			data_data_saving_path = "./chi2pp/datadata/eta_%i.png";
		} 
	...
	for (int runperiod = 0; runperiod < 22; runperiod++){
		...
		dMass_Err_pp[runperiod] = 0;
		dWidth_Err_pp[runperiod] = 0;
		...
		if (isbk) {
			...
			h_data_bksub_pp[runperiod]->Draw("P HIST");
			h_mc_signal_pp[14][0][runperiod]->Draw("P SAME");
		}
		if (!isbk){
	...
	temp->cd();
	if (isbk){
		g_pp_dmass->Write("run_period_dM_chi2_bksub",2);
		g_pp_dwidth->Write("run_period_dWidth_chi2_bksub",2);
	}
	else{
		g_pp_dmass->Write("run_period_dM_chi2",2);
		g_pp_dwidth->Write("run_period_dWidth_chi2",2);
	}
	temp->Close();
