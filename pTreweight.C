...
void drawandsave(TFile *f1, TH1D *h_PbPb, TH1D *h_pp, TH1D *h_ratio, bool iseta)
{
    // Create Canvas and Pads
    TCanvas *c = new TCanvas("c", "PbPb / pp", 800, 800);
    ...
    leg1->AddEntry(h_PbPb, "Pb+Pb (5.02 TeV)", "l");
    leg1->AddEntry(h_pp, "p+p (13 TeV)", "l");
    leg1->Draw();
    ...
    fitFunc->SetParameters(3.2, 14.0, 0.3); // Initial parameters
    ...
    if (!iseta)
    {
        ...
        c->SaveAs("./pTratio/ratio_plot_FA.pdf");
    }
    else
    {
        fitFunc->Write("Eta_ratio_fit",2);
        c->SaveAs("./pTratio/ratio_plot_ETA.pdf");
    }
}
